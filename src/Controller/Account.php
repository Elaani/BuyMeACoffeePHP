...
use BuyMeACoffee\Service\UserValidation;
            ...
            if (isset($fullName, $email, $password)) {
                if (
                    ...
                    if ($this->userService->doesAccountEmailExist($email)) {
                        $viewVariables[View::ERROR_MESSAGE_KEY] = 'An account with that email address already exist.';
                    } else {
                        $user = [
                            'fullname' => $fullName,
                            'email' => $email,
                        ...
                        if ($userId = $this->userService->create($user)) {
                            ...
                            $viewVariables[View::ERROR_MESSAGE_KEY] = 'An error has occurred while creating your account. Please try again later.';
                        }
                    }
                } else {
            ...
            if (isset($name, $email)) {
                ...
                if ($hasEmailChanged) {
                    if (! $this->userValidation->isEmailValid($email) || $this->userService->doesAccountEmailExist($email)) {
                        $viewVariables[View::ERROR_MESSAGE_KEY][] = 'Email is incorrect or already exists in the system';
                    } else {
